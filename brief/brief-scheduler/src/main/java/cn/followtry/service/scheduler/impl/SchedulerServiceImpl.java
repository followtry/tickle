package cn.followtry.service.scheduler.impl;
...
import cn.followtry.service.scheduler.base.SchedulerManager;
...
import cn.followtry.service.scheduler.core.job.JobListener;
import cn.followtry.service.scheduler.core.job.MyJobScheduler;
import cn.followtry.service.scheduler.core.job.QuartzJobFactory;
import cn.followtry.service.scheduler.core.job.SimpleJob;
...
*/
    ...
    Trigger trigger = SchedulerManager.getNewSimpleTrigger(simpleScheduleBuilder,triggerKey,jobData,
            triggerTime);
  ...
  public void work(ScheduleJob scheduleJob) throws ParseException {
    ...
    startCronJobSchedule(scheduleJob.getJobDataMap(),cronScheduleBuilder,triggerKey,scheduleJob
            .getTriggerEndTime());
  }
   ...
   */
  private void startCronJobSchedule(JobDataMap jobData,CronScheduleBuilder scheduleBuilder,TriggerKey
          triggerKey,Date triggerEndTime) {
    Trigger newTrigger = SchedulerManager.getNewCronTrigger(scheduleBuilder,triggerKey,jobData,
            triggerEndTime);
  ...
  public void scheduleThroughputDataFlowJob(ScheduleJob scheduleJob) {
    ...
    JobConfiguration jobConfig = ElasticJobSchedulerManager.getJobConfig(scheduleJob.getJobName(),
            BatchThroughputDataFlowJob.class,scheduleJob.getShardingTotalCount(),scheduleJob
                    .getCronExpression(),jobParams);
    zkRegCenter.init();
  ...
  public void scheduleSimpleJob(ScheduleJob scheduleJob) {
    ...
    JobConfiguration jobConfig = ElasticJobSchedulerManager.getJobConfig(scheduleJob.getJobName(),SimpleJob
            .class,scheduleJob.getShardingTotalCount(),scheduleJob.getCronExpression(),jobParams);
