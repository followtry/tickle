...
public class BeanValidator implements ConstraintValidator {
	...
	private static Map<Class<?>, BeanValidator> beanValidatorRegisterMap = new ConcurrentHashMap<Class<?>, BeanValidator>();

	/**
	 * 创建 BeanValidator 的实例；
	 * 
	 * 如果指定的类型不是一个符合 Java Bean 规范的类型，或者其没有标注了约束的属性，则返回 null；
	 * 
	...
	public static BeanValidator create(Class<?> beanType) {
		...
		try {
			...
			BeanPropertyValidator[] propValidatorChain = propValidators
					.toArray(new BeanPropertyValidator[propValidators.size()]);
	...
	private static void trace(Class<?> beanType) {
		...
		if (beanSet.contains(beanType)) {
			throw new IllegalStateException(
					"There are cycle dependencies for the constraint validation of bean[" + beanType.getName() + "]!");
		}
